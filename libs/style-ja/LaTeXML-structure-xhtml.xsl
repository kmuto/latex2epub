...
<xsl:stylesheet
  ...
  <xsl:template match="ltx:keywords" xml:space="preserve">
    <div class='{f:classes(.)}'><i>キーワード: </i><xsl:apply-templates/></div>
  </xsl:template>
  ...
  <xsl:template match="ltx:classification">
	...
	<xsl:otherwise>分類</xsl:otherwise>
      </xsl:choose>: </i>
  ...
  <xsl:template match="ltx:chapter/ltx:title">
    <xsl:call-template name="maketitle">
      ...
      <xsl:with-param name="title_prefix">第</xsl:with-param>
      <xsl:with-param name="title_suffix">章</xsl:with-param>
    </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template match="ltx:section/ltx:title">
    <xsl:call-template name="maketitle">
      ...
      <xsl:with-param name="title_suffix"></xsl:with-param>
    </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template match="ltx:appendix/ltx:title">
    <xsl:call-template name="maketitle">
      <xsl:with-param name="title_level" select="$section_level"/>
      <xsl:with-param name="title_prefix">付録</xsl:with-param>
    </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template name="maketitle">
	...
	<xsl:if test="$title_suffix"><xsl:value-of select="$title_suffix"/><xsl:text> </xsl:text></xsl:if>
      </xsl:if>
      <xsl:apply-templates/>
    </xsl:element>
  </xsl:template>
